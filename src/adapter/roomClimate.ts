...
export async function createRoomClimateState(
  adapter: Innoxel,
  ...
  await Promise.all([
    createState("operatingState", "string", {
      role: "string",
      states: ["heating", "cooling"],
    }),
    createState("valveState", "string", {
      role: "string",
      states: ["open", "closed"],
    }),
    createState("valveOpen", "boolean", { role: "indicator" }),
    createState("actualTemperatureMean", "number", {
...
export async function updateRoomClimate(
  adapter: Innoxel,
  data: IModuleRoomClimate[],
): Promise<void> {
  const channelPromises = data.map(async (module) => {
    ...
    for (const key of Object.keys(module.thermostat)) {
      const obj =
        module.thermostat[key as keyof IModuleRoomClimate["thermostat"]];

      if (key === "valveState")
        statePromises.push(
          adapter.setStateChangedAsync(
            `roomClimate.${module.index}.valveOpen`,
            obj === "open",
            true,
          ),
        );

      let value: ioBroker.StateValue | ioBroker.SettableState =
